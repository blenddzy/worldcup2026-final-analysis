...
from src.fox_sports_client import load_team_stats, load_per_match_stats
...
def run_pipeline(config_path="config.yaml"):
    ...
    print("[1/8] Fetching openfootball data...")
    ...
    print("[2/8] Loading tournament matches...")
    ...
    print("[3/8] Extracting final goals...")
    ...
    print("[5/8] Fetching Fox Sports team stats...")
    ...
    print("[6/8] Fetching Fox Sports per-match advanced stats...")
    per_match_path = "data/external/fox_sports_per_match_stats.json"
    per_match_stats = load_per_match_stats(per_match_path)
    if per_match_stats:
        print(f"       Loaded {len(per_match_stats)} matches with real advance stats")
    else:
        print("       Per-match stats not available, will use synthetic data")

    print("[7/8] Generating plots...")
    plot_paths = generate_all_plots(final, matches_df, standings, raw_data=of_data)

    print("[8/8] Generating conclusion...")
